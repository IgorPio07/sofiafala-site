<?php
require_once("resources/library/funcoes.php");

$tipo = $_GET['tipo'];

if ($tipo == 'projetosofia') {
    $objpath = new funcoes();
    $path = $objpath->getsofiafalaarquivos();
}

$arquivo = $path.$_GET["arquivo"];

//echo 'aqui'.$arquivo;
$nomeoriginal = $_GET["arquivo"];
...
if (isset($arquivo) && file_exists($arquivo)) { // faz o teste se a variavel não esta vazia e se o arquivo realmente existe
    ...
    header("Content-Disposition: attachment; filename=" . basename($nomeoriginal)); // informa ao navegador que é tipo anexo e faz abrir a janela de download, tambem informa o nome do arquivo
