...
<form id="formDados" name="formDados" method="POST" action="doacao_voz.php">
  <div class="form-row">
      ...
      <input type="hidden" class="form-control" id="tIdCad" name="tIdCad" value="" readonly>
...
*/
  ...
  if(nasc.value == ""){
    ...
    $('#exampleModal').find('.modal-title').text('Atenção!');
    $('#exampleModal').find('.modal-body').text('Data de nascimento é obrigatória.');
    $('#exampleModal').modal('show');
    return false;
  }
  ...
  if(nasc.value.length != 10){
    //alert("Data de nascimento é obrigatório");
    $('#exampleModal').find('.modal-title').text('Atenção!');
    $('#exampleModal').find('.modal-body').text('Data de nascimento inválida.');
    $('#exampleModal').modal('show');
    return false;
  }
...
document.getElementById("inscricao").innerHTML = "ID: "+ tID.value;
